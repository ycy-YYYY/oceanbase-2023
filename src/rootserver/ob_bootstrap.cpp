...
int ObBootstrap::execute_bootstrap(rootserver::ObServerZoneOpService &server_zone_op_service)
{
  ...
  if (OB_SUCC(ret)) {
    ...
    } else if (OB_FAIL(ddl_service_.refresh_schema_async(OB_SYS_TENANT_ID))) {
...
int ObBootstrap::parallel_batch_create_schema(ObDDLService &ddl_service, ObIArray<ObTableSchema> &table_schemas){
  ...
  for (int64_t i = 0; OB_SUCC(ret) && i < data_tables.count(); ++i) {
    if (data_tables.count() == (i + 1) || (i + 1 - begin) >= batch_count) {
      ...
      std::thread th([&, begin, i, cur_trace_id] () {
        ...
        int64_t retry_times = 1;
        while (OB_SUCC(ret)) {
          if (OB_FAIL(batch_create_schema_local(OB_SYS_TENANT_ID, ddl_service, data_tables, begin, i + 1))) {
  ...
  if (finish_cnt != table_schemas.count()) {
    ret = OB_ERR_UNEXPECTED;
    LOG_WARN("parallel_create_table_schema fail", K(finish_cnt), K(table_schemas.count()), K(ret), K(OB_SYS_TENANT_ID));
  }
  return ret;
}

int ObBootstrap::batch_create_schema_local(uint64_t tenant_id,
                              ObDDLService &ddl_service,
                              ObIArray<ObTableSchema> &table_schemas,
                              const int64_t begin, const int64_t end)
{
